...
const GoalList: React.FC<GoalListProps> = ({
  ...
  const groupedGoals = goals.reduce<Record<string, Goal[]>>((acc, goal) => {
    const categoryName = goal.category ? goal.category.name : "Uncategorized";
    if (!acc[categoryName]) {
      acc[categoryName] = [];
    }
      ...
      {Object.keys(groupedGoals).map((categoryName) => (
        <div key={categoryName} className="mb-4">
          ...
          >
            ...
            <span
              className={`transform transition-transform duration-300 ${openCategories[categoryName] ? "rotate-90" : ""}`}
            >
              &#9656;
            </span>
          </button>
                ...
                >
                  <div>
                    ...
                    <p className="text-gray-500">
                      Current: {Number(goal.current_value)} / Target: {Number(goal.target)}
                    </p>
                  </div>
                  <button
